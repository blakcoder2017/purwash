...
import { getActiveJobs, updateOrderStatus } from "../api/partner";

const nextStatusMap: Partial<Record<OrderStatus, OrderStatus>> = {
  assigned: "dropped_at_laundry",
  dropped_at_laundry: "washing",
  washing: "ready_for_pick",
};

const statusLabel: Partial<Record<OrderStatus, string>> = {
  assigned: "Confirm received",
  dropped_at_laundry: "Start washing",
  washing: "Mark ready",
};
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={!loading ? <Text style={styles.empty}>No active jobs.</Text> : null}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Order #{item.friendlyId}</Text>
            <Text style={styles.meta}>Status: {item.status.replace(/_/g, " ")}</Text>
            <Text style={styles.meta}>Address: {item.location?.addressName || "-"}</Text>
