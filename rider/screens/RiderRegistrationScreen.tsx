...
interface StepTwoData {
  ...
  vehicleType?: 'motorcycle' | 'bicycle' | 'car' | 'Keke';
...
const RiderRegistrationScreen: React.FC = () => {
  ...
  const handleStepOne = async () => {
    ...
    try {
      const response = await riderApi.partnerRegistration.stepOne(stepOneData);
      if (response.success) {
  ...
  const handleStepTwo = async () => {
    ...
    try {
      const response = await riderApi.partnerRegistration.stepTwo(stepTwoData, tempToken);
      if (response.success) {
        setTempToken(response.data.tempToken);
        setCurrentStep(3);
      } else {
  ...
  const handleStepThree = async () => {
    ...
    try {
      const response = await riderApi.partnerRegistration.stepThree(stepThreeData, tempToken);
      if (response.success) {
  ...
  const renderStepTwo = () => (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold text-primary mb-6">Rider Information</h2>
      
      <input
        type="text"
        placeholder="Business Name (e.g., QuickRide Services)"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Service Area Location</label>
        <LocationPicker
      ...
      <textarea
        placeholder="Tell us about your riding experience (optional)"
      ...
      <div>
        ...
        <div className="flex gap-4">
          {(['Keke','motorcycle', 'bicycle'] as const).map((vehicle) => (
            <button
              key={vehicle}
              type="button"
              onClick={() => setStepTwoData({ ...stepTwoData, vehicleType: vehicle })}
              className={`flex-1 p-3 rounded-md font-bold transition-colors capitalize ${
                stepTwoData.vehicleType === vehicle 
      ...
      <input
        ...
        value={stepTwoData.vehicleNumber}
        onChange={(e) => setStepTwoData({ ...stepTwoData, vehicleNumber: e.target.value })}
        className={inputClasses}
      />
  ...
  const renderStepThree = () => (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold text-primary mb-6">Payment & Safety Setup</h2>
      ...
      <div className="border-t pt-4">
        ...
        <input
          ...
          value={stepThreeData.emergencyContact?.name}
          onChange={(e) => setStepThreeData({ 
            ...stepThreeData, 
            emergencyContact: { ...stepThreeData.emergencyContact, name: e.target.value } 
          })}
          className={inputClasses}
          required
        />
        ...
        <input
          ...
          value={stepThreeData.emergencyContact?.phone}
          onChange={(e) => setStepThreeData({ 
            ...stepThreeData, 
            emergencyContact: { ...stepThreeData.emergencyContact, phone: e.target.value } 
          })}
          className={inputClasses}
          required
        />
        ...
        <input
          ...
          value={stepThreeData.emergencyContact?.relationship}
          onChange={(e) => setStepThreeData({ 
            ...stepThreeData, 
            emergencyContact: { ...stepThreeData.emergencyContact, relationship: e.target.value } 
          })}
          className={inputClasses}
          required
        />
    ...
    <div className="min-h-screen bg-primary flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-300">Rider Registration</p>
          <p className="text-sm text-gray-400 mt-2">Join our delivery network and start earning</p>
        </div>
